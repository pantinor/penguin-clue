...
public class BamAnimationStore {
    ...
    private Map<String, Animation> animations = new LinkedHashMap<>();
    private ArrayList<Bam> bams = new ArrayList<>();
    ...
    private int maxFramesInSeq = 0;
    int maxWidth;
    int maxHeight;

    private  static final int MIN_FRAMES_PER_ANIM = 5;
    private  static final int MAX_SEQUENCES = 95;
    ...
    public void init() {
        ...
        try {
            ...
            int seqnum = 0;
            Map<String, Array<TextureRegion>> arrays = new LinkedHashMap<>();
            ...
            for (Bam bam : bams) {
                ...
                for (int i = 0; i < anims.length; i++) {
                    ...
                    for (int j = 0; j < anim.frameCount; j++) {
                        ...
                        arr.add(new TextureRegion(fr));
                    }
                    seqnum++;
                }
            }

            animations = sortAnimations(arrays);
    ...
    private Map<String, Animation> sortAnimations(Map<String, Array<TextureRegion>> input) {
        SortedSet<String> keys = new TreeSet<>(input.keySet());
        LinkedHashMap<String, Animation> sortedMap = new LinkedHashMap<>();
        for (String key : keys) {
            sortedMap.put(key, new Animation(DURATION, input.get(key), PlayMode.LOOP));
        }
        return sortedMap;
    }

    private static String getAnimationName(String fileName, String defaultName) {
    ...
    private static Collection<File> getFiles(String directoryName, String filter) {
    ...
    private class Frame {
        ...
        Frame(byte[] buffer, int offset) {
            ...
            Pixmap ib = new Pixmap(width, height, Pixmap.Format.RGBA8888);
            ib.setColor(0f, 0f, 0f, 1f);
            ib.fillRectangle(0, 0, width, height);
            ...
            for (int h_idx = 0; h_idx < height; h_idx++) {
                for (int w_idx = 0; w_idx < width; w_idx++) {
                    ...
                    ib.drawPixel(w_idx, h_idx, rest);

                }
            }

            this.image = new Texture(ib);
            ib.dispose();
