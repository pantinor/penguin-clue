...
/**
 * Combines separate image files into a single tileset, exporting a image array
 * and position/extent metadata.
 * 
 * @author pheinrich
 * @version 1
 */
public class TilePacker {
	 ...
	 * parameter will be filled with the resultant location (within the new
	 * tile) of each individual image.
	 * 
	 * @param aImages
	 *            an array of source images
	 * @param aExtents
	 *            an array of rectangles to receive position information
	 */
	...
	public static void main(String[] asArgs) {
		try {
			...
			// Scan the command line arguments for source images and an output
			// name.
			for (int i = 0; i < asArgs.length; i++) {
				if (asArgs[i].equalsIgnoreCase("-h")) {
					...
					// Set the output filename. Subsequent occurrences of the -o
					// switch will
					// override the value we set here.
					sOutPath = asArgs[++i];
				} else if (asArgs[i].equalsIgnoreCase("-w")) {
					// Set the maximum tile width. Subsequent occurrences of the
					// -w switch will
					// override the value we set here.
			...
			if (0 < lFiles.size()) {
				...
				for (int i = 0; i < lFiles.size(); i++) {
					...
					try {
						...
						// The ImageIO library complains if the number of
						// palette entries doesn't
						// equal 2^BPP, where BPP is the bits-per-pixel. (This
						// is an error in the
						// library, however; the PNG format can certainly handle
						// the situation.)
						System.out.println("Chunk problem with " + file.getAbsolutePath() + ".  Check palette size against BPP.");
					}
				}
				...
				// Generate a "tiled" image from the source images specified.
				// The image order
				// within the array parameter may change, but the corresponding
				// extent info
				...
				// Write out the metadata. This could be more elegant, but is it
				// necessary?
	 ...
	 * 
	 * @param nMaxWidth
	 *            the maximum desired tile size
	 * @return a <code>BufferedImage</code> containing all the source images
	 *         combined together
	 * @throws IOException
	 *             if at least one image exceeds the maximum width
	 */
	public BufferedImage pack(int nMaxWidth) throws IOException {
		...
		// Now shuffle the images into progressively wider tiles, starting with
		// one
		// as wide as the widest individual image, but never wider than the
		// maximum
		// specified by the caller.
		while (nWidth < nMaxWidth) {
			int nArea = rearrangeForWidth(nWidth);

			// Choose the width resulting in the smallest area and shortest
			// perimeter.
			if (nArea < nMinArea || (nArea == nMinArea && (nWidth + (nArea / nWidth)) < (nBestWidth + (nMinArea / nBestWidth)))) {
				nMinArea = nArea;
				nBestWidth = nWidth;
			}
	...
	private int rearrangeForWidth(int nWidth) {
		...
		// The destination bitmap starts as one huge hole (new images may be
		// placed
		// anywhere). As we add images, the hole shrinks and new holes are
		// added.
		// These will usually overlap at least one existing hole in one
		// direction.
		//
		...
		// Process all images sequentially (they're already arranged in
		// decreasing
		// order by total area).
		for (int i = 0; i < m_aImages.length; i++) {
			BufferedImage image = m_aImages[i];
			...
			// Compare the current image to every hole, looking for one big
			// enough to hold
			// it. The holes are arranged by vertical position (highest first),
			// then
			...
			for (int j = 0; j < lHoles.size(); j++) {
				...
				if (rHole.width >= rExtent.width && rHole.height >= rExtent.height) {
					...
					// This one will do, so position the image in the upper left
					// corner of the
					// hole we found.
					rExtent.setLocation(rHole.x, rHole.y);

					// Now compare the image--in its new position--to all the
					// holes we know about,
					// because its presence may require some to be broken into
					// smaller holes.
					for (int k = 0; k < lHoles.size(); k++) {
						Rectangle rScan = (Rectangle) lHoles.get(k);

						// Do nothing if the image doesn't overlap the current
						// hole.
						if (rScan.intersects(rExtent)) {
							// Top edge of image (in new position) is below this
							// hole's top;
							...
							// Left edge of image is to the right of this hole's
							// left edge;
							...
							// Right edge of image is to the left of this hole's
							// right edge;
					...
					// Trim down the new list of holes by pruning those that are
					// wholly contained.
					lHoles.clear();
	...
	/**
	 * Copies bounded extents from one list to another, pruning any that are
	 * wholly contained. The resulting collection will be sorted in order of
	 * ascending rows and columns, i.e. sort low-y then low-x.
	 * 
	 * @param lFrom
	 *            the source list of <code>Rectangle</code> objects
	...
	private List collapseHoles(ArrayList lFrom) {
		...
		for (int i = 0; i < lFrom.size(); i++) {
			...
			// null is a valid value, since we may remove holes from list as
			// they're
			...
			if (null != hole) {
				for (int j = 1 + i; j < lFrom.size(); j++) {
					...
					// Above comment applies to all following holes, too, of
					// course.
					if (null != comparand) {
						...
						// If, when taken together, the holes cover the same
						// area (or less) as when
						// taken separately, either they're equal, one contains
						// the other, or they
		...
		// Initialize an array of the holes that remain (they're non-null) so we
		// can
