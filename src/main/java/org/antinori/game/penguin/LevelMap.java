...
public class LevelMap implements TileBasedMap {
	...
	private void generate(Map map) {
		...
		setTile(map.getEndRoom().getX() + (map.getEndRoom().getWidth() / 2), map.getEndRoom().getY() + (map.getEndRoom().getHeight() / 2), 0, 17);
	}
	...
	public boolean blocksSight(int tx, int ty) {
	...
	public boolean hasLom(float x1, float y1, float x2, float y2, float size) {
		...
		float dx = (x2 - x1);
		float dy = (y2 - y1);
		float len = Math.max(Math.abs(dx), Math.abs(dy)); // (float)
		...
		for (int y = sectiony; y < sectiony + height; y++) {
			for (int x = sectionx; x < sectionx + width; x++) {
				...
				if (tile != 0) {
					...
					int tx = tile % 16;
					int ty = tile / 16;
					//System.out.println("drawingtile \t"+tx+"\t"+ty);
	...
	private void drawQuads(LightMap lightMap, int x, int y, Image image, int tile, boolean walls) {
		...
		image.drawEmbedded((x * PenguinGame.TILE_SIZE), (y * PenguinGame.TILE_SIZE), PenguinGame.TILE_SIZE, PenguinGame.TILE_SIZE);
	}

	private void configureLights(LightMap lightMap, Image image, int corner, int x, int y) {
		image.setColor(corner, lightMap.getRed(x, y), lightMap.getGreen(x, y), lightMap.getBlue(x, y), 1);
	}
