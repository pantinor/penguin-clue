...
public class Map {
	...
	public int[] generateRandomRoomLocation() {
		Room room = pickRandomRoom();
		while (room == null) {
			room = pickRandomRoom();
		}
		int randomRoomX = room.getX() + (room.getWidth()/2);
		int randomRoomY = room.getY() + (room.getHeight()/2);
		int[] coords = {randomRoomX, randomRoomY};
		return coords;
	}
