...
import org.antinori.game.penguin.Light;
...
public class Actor extends Spatial implements Mover, DungeonElement {
	...
	public Actor(World world, Entity owner, PenguinGame game, GameContainer container, LevelMap map, ActorType type, int direction) {
	...
	public void initialize() {
		...
		light = new Light(transform.getX(),transform.getY(), 4, 2);
	}
	...
	public void update(LevelMap map) {
		if (currentPath != null) {
			...
			light.position(transform.getX(),transform.getY());			
		} else {
